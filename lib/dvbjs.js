        ...
        request(url, function(err, res, data){
            ...
            for (var i = 0; i < data.trips.length; i++) {
                ...
                for (var j = 0; j < data.trips[i].legs.length; j++) {
                    ...
                    // var path            = convertPath(data.trips[i].legs[j].path); // the code works, but it isn't showing up in the node object when I add it in below... Why?
...
function convertPath(path) {
    ...
    for (var i = 0; i < coords.length; i++) {
        coords[i] = convertCoords(coords[i]);
    }
};
