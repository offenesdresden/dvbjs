        ...
        request(url, function(err, res, data){
            ...
            for (var i = 0; i < data.trips.length; i++) {
                ...
                for (var j = 0; j < data.trips[i].legs.length; j++) {
                    trip.nodes.push({mode: data.trips[i].legs[j].mode.name});
                }
            ...
            if (Array.isArray(data.stopFinder.points)) {
                for (var i = 0; i < data.stopFinder.points.length; i++) {
                    if (data.stopFinder.points[i].posttown === 'Dresden') {
                        json.results.push({stop: data.stopFinder.points[i].object, coords: data.stopFinder.points[i].ref.coords});
                    }
                }
            } else {
                if (data.stopFinder.points.point.posttown === 'Dresden') {
                    json.results.push({stop: data.stopFinder.points.point.object, coords: data.stopFinder.points.point.ref.coords});
                }
            }
