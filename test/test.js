...
describe('dvb.pins', function () {
    ...
    describe('dvb.pins "51.026578, 13.713899, 51.035565, 13.737974, POI+PLATFORM"', function () {
        mockRequest('pins-poi-pf.json');

        it('should contain objects with pinType of RENT_A_BIKE or PLATFORM', function (done) {
            var hasPOI = false, hasSTOP = false;

            dvb.pins(51.026578, 13.713899, 51.035565, 13.737974, [dvb.pins.type.RENT_A_BIKE, dvb.pins.type.PLATFORM])
                .then(function (data) {
                    data.forEach(function (elem) {
                        if (elem.type == dvb.pins.type.RENT_A_BIKE)
                            hasPOI = true;
                        else if (elem.type == dvb.pins.type.PLATFORM)
                            hasSTOP = true;
                        else
                            assert.fail(elem.type, dvb.pins.type.RENT_A_BIKE + " || " + dvb.pins.type.PLATFORM, "pinType should be '" + dvb.pins.type.RENT_A_BIKE + " or '" + dvb.pins.type.PLATFORM + "'");
                    });
                    assert(hasPOI && hasSTOP, "pinTypes should be '" + dvb.pins.type.RENT_A_BIKE + " and '" + dvb.pins.type.PLATFORM + "'");
